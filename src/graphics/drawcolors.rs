...
impl DrawColors {
    pub fn new() -> &'static mut Self {
        unsafe { &mut *(DRAW_COLORS as *mut DrawColors) }
    }
...
// pub const A: &DrawColors = unsafe { &*(DRAW_COLORS as *const DrawColors) };

#[cfg(test)]
mod tests {
    use super::DrawColors;

    #[test]
    fn get() {
        let draw_colors = DrawColors(0x3102);
    ...
    #[test]
    fn set() {
        let mut draw_colors = DrawColors(0);
    ...
    fn set_one_first_existing() {
        let mut draw_colors = DrawColors(0x1111);
        draw_colors.set_one(0, 3);
        let actual = draw_colors.get();
        let expected = 0x3111;
        assert_eq!(actual, expected);
